...
export class ShaderProgram {
  ...
  private createShader(type: number, source: string): WebGLShader {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      ...
      throw new Error(`Shader compile error (${shaderType}): ${info}`);
    }
...
export class FrameBuffer {
  ...
  private createFramebuffer() {
    ...
    const hasFloatSupport = gl.getExtension("EXT_color_buffer_float");
    const internalFormat = hasFloatSupport ? gl.RGBA16F : gl.RGBA8;
    const type = hasFloatSupport ? gl.FLOAT : gl.UNSIGNED_BYTE;
  ...
  public resize(width: number, height: number): void {
    ...
    // Check if float textures are supported
    const hasFloatSupport = this.gl.getExtension("EXT_color_buffer_float");
    const internalFormat = hasFloatSupport ? this.gl.RGBA16F : this.gl.RGBA8;
    const type = hasFloatSupport ? this.gl.FLOAT : this.gl.UNSIGNED_BYTE;
...
export class MultiPassRenderer {
  ...
  constructor(canvas: HTMLCanvasElement, configs: RenderPassConfig[]) {
    ...
    // 检查浮点纹理扩展 (optional - fallback to RGBA8 if not available)
    const ext = gl.getExtension("EXT_color_buffer_float");
    if (!ext) {
      console.warn("EXT_color_buffer_float not supported, using RGBA8 fallback");
    }
